...
- **Messaging:** End-to-end encrypted messaging via XMTP React Native SDK v3. Secure remote signing architecture ensures private keys remain encrypted on the server. Web platform gracefully falls back to mock data due to XMTP's native module requirements. Unified chat persistence combines XMTP conversations and local AsyncStorage chats. Real-time message streaming with proper cleanup using SDK's `cancelStream()` method to prevent listener leaks during tab switches. Debounced conversation list updates (500ms) for performance.
...
- **Revenue System:** Treasury-first model with weekly withdrawals. Database tables: revenue_ledger (all platform fees with collection status), creator_balances (creator earnings), and creator_withdrawals (payout history). Tips: 100% goes to PLATFORM_TREASURY_ADDRESS (creators see balance increase in real-time, can withdraw once per week on Mondays UTC). P2P Transfers: 1% fee to treasury, 99% to recipient. Creator Earnings dashboard shows total earned, available balance, next withdrawal window countdown, and withdrawal history.
